...
const getSingleUser = async (userId: string): Promise<TUser | null> => {
  const result = await User.findById(userId).populate('partner', 'name email profileImage').lean();
...
const updateUser = async (userId: string, payload: Partial<TUser>): Promise<TUser | null> => {
  ...
  const updatedUser = await User.findByIdAndUpdate(userId, payload, { new: true }).lean();
  if (!updatedUser) {
    throw new ApiError(StatusCodes.NOT_FOUND, 'User not found');
  }
