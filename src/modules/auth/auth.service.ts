...
const generateUserCode = (prefix: string = '', length: number = 6): string => {
...
const createUser = async (userData: any) => {
  ...
  while (!isUnique) {
    code = generateUserCode(); // Using Option 1
    const existingRef = await User.findOne({ code } as Partial<TUser>);
    if (!existingRef) isUnique = true;
  }

  const user = await User.create({ ...userData, code, role: 'user', isEmailVerified: true } as Partial<TUser>);
  if (!user) {
